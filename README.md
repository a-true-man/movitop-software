...
3. **ישאל כמה GB זיכרון להקצות** לבניית המנוע (ברירת מחדל: 12) – הזינו בהתאם ל‑RAM הפנוי במחשב
4. יבנה את גרף המסלולים (זה השלב שצורך הרבה זיכרון)
5. ייצור קובץ `stops.json` לחיפוש תחנות
6. ינקה קבצים זמניים
...
בניית הגרף עובדת על מיליוני נתיבים ותחנות. התהליך עושה חישובים כבדים – 20–60 דקות הן טווח זמן רגיל. **הקצאת יותר זיכרון** (RAM) למנוע בזמן הבנייה יכולה להאיץ את התהליך – הסקריפט שואל כמה GB להקצות (ברירת מחדל: 12).

### האם הנתונים מתעדכנים אוטומטית?
לא. יש להריץ מחדש את `npm run deploy` **כל שבוע** (או בתדירות שעליה מחליטים) כדי להוריד GTFS מעודכן ולבנות גרף חדש. כך לוח הזמנים יישאר עדכני.
